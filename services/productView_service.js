...
async function getAllProductView(params){
    ...
    try { //console.log("llllllego",params);
        const {sucursalId} = params;
        const res=await viewProductos.findAll({where:{idsucursal:sucursalId}});
        if(res){
            response.data=res;
...
async function getAllProductViewResume(params){
    ...
    try { //console.log("llllllego",params);
        const {sucursalId} = params;
        const res=await viewProductos.findAll({where:{idsucursal:sucursalId}, attributes: ['id', 'nombre', 'nomtamanio', 'nompresentacion'],});
        if(res){
            response.data=res;
            response.message="products found by this sucursal";
            response.status=200;
